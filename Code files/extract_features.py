...
#####################
veil_color_ranges = [
    (np.array([93, 123, 114]), np.array([135, 148, 133]))  #LAB ranges for blue white veils
] 
...
def color_matching(image, color_ranges = veil_color_ranges):
    """ Match colors in the image to a predefined set of color ranges and return mask of matched areas. """
    matched_image = np.zeros(image.shape[:2], dtype=np.uint8)
    image_flat = image.reshape((-1, 3))
    
    for (min_lab, max_lab) in color_ranges:
        # Create masks for each range
        in_range_mask = np.all((image_flat >= min_lab) & (image_flat <= max_lab), axis=1)
        matched_image.flat[in_range_mask] = 255
        
    return matched_image


def process_bwv(image, mask, color_ranges = veil_color_ranges, area_ratio = 0.08):
    try:
        
        image_lab = convert_to_lab(image)
        veil_detected = color_matching(image_lab)
        
        # Apply mask to the detected veil
        masked_veil_detected = cv2.bitwise_and(veil_detected, veil_detected, mask=mask)
        
        # Calculate the percentage of matched area within the masked region
        matched_area = np.sum(masked_veil_detected == 255)
        masked_area = np.sum(mask > 0)  # Only consider areas where the mask is applied
        has_veil = matched_area > (masked_area * area_ratio)

        # Debugging output
        #print(f"Image: {image_path}, Matched Area: {matched_area}, Masked Area: {masked_area}, Ratio: {matched_area/masked_area:.2%}")

        return [int(has_veil)]
    except Exception as e:
        return None

def extract_features(image,mask, image_for_veil):
    features_imagex = get_asymm_results(mask) + find_colors(image, mask) + process_bwv(image_for_veil, mask)
